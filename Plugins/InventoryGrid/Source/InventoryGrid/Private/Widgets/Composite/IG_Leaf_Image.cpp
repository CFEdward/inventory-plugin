...
void UIG_Leaf_Image::SetImage(UTexture2D* Texture) const
{
	Image_Icon->SetBrushFromTexture(Texture);
}

void UIG_Leaf_Image::SetImageSize(const FVector2D& Size) const
{
	Image_Icon->SetDesiredSizeOverride(Size);
}

FVector2D UIG_Leaf_Image::GetImageSize() const
{
	return Image_Icon->GetDesiredSize();
}

void UIG_Leaf_Image::SetBoxSize(const FVector2D& Size) const
{
